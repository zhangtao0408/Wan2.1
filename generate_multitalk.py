...
def _parse_args():
    ...
    parser.add_argument(
        ...
        help="The path to the Kokoro Checkpoint Directory.")
    parser.add_argument(
        "--quant_dir",
        type=str,
        default=None,
...
def process_tts_single(text, save_dir, voice1, kokoro_dir):    
    s1_sentences = []
...
def process_tts_multi(text, save_dir, voice1, voice2, kokoro_dir):
...
def generate(args):
    ...
    if world_size > 1:
        ...
        assert not (
            args.vae_parallel
        ), f"vae_parallel is not supported in non-distributed environments."
    ...
    with open(args.input_json, 'r', encoding='utf-8') as f:
        ...
        if args.audio_mode=='localfile':
            ...
            if 'human2_voice' not in input_data['tts_audio'].keys():
                new_human_speech1, sum_audio = process_tts_single(input_data['tts_audio']['text'], args.audio_save_dir, input_data['tts_audio']['human1_voice'], kokoro_dir=args.kokoro_dir)
                audio_embedding_1 = get_embedding(new_human_speech1, wav2vec_feature_extractor, audio_encoder)
                emb1_path = os.path.join(args.audio_save_dir, '1.pt')
                torch.save(audio_embedding_1, emb1_path)
                input_data['cond_audio']['person1'] = emb1_path
                ...
                new_human_speech1, new_human_speech2, sum_audio = process_tts_multi(input_data['tts_audio']['text'], args.audio_save_dir, input_data['tts_audio']['human1_voice'], input_data['tts_audio']['human2_voice'], kokoro_dir=args.kokoro_dir)
                audio_embedding_1 = get_embedding(new_human_speech1, wav2vec_feature_extractor, audio_encoder)
                audio_embedding_2 = get_embedding(new_human_speech2, wav2vec_feature_extractor, audio_encoder)
                emb1_path = os.path.join(args.audio_save_dir, '1.pt')
                emb2_path = os.path.join(args.audio_save_dir, '2.pt')
