...
class Generator(nn.Module):
    ...
    def forward(self, x, s, f0):
        ...
        for i in range(self.num_upsamples):
            # 1. Leaky ReLU (如果是复数，分别处理实部虚部)
            if x.is_complex():
                x = torch.complex(F.leaky_relu(x.real, 0.1), F.leaky_relu(x.imag, 0.1))
            else:
                x = F.leaky_relu(x, negative_slope=0.1)

            # 2. Noise Convs (你之前修过的地方)
            conv = self.noise_convs[i]
            if har.is_complex():
                real_out = conv(har.real)
                imag_out = F.conv1d(har.imag, conv.weight, None, conv.stride, conv.padding, conv.dilation, conv.groups)
                x_source = torch.complex(real_out, imag_out)
            else:
                x_source = conv(har)

            # 3. Noise Res (你之前修过的地方)
            res_layer = self.noise_res[i]
            if x_source.is_complex():
                res_real = res_layer(x_source.real, s)
                res_imag = res_layer(x_source.imag, s)
                x_source = torch.complex(res_real, res_imag)
            else:
                x_source = res_layer(x_source, s)

            # 4. Upsample (新增补丁：防止下一轮循环报错)
            ups_layer = self.ups[i]
            if x.is_complex():
                # ConvTranspose1d 也要拆分
                ups_real = ups_layer(x.real)
                ups_imag = ups_layer(x.imag) # 假设没有 Bias，如果有需检查
                x = torch.complex(ups_real, ups_imag)
            else:
                x = ups_layer(x)
            ...
            for j in range(self.num_kernels):
                ...
                #拆分输入
                if x.is_complex():
                    blk_real = blk(x.real, s)
                    blk_imag = blk(x.imag, s)
                    blk_out = torch.complex(blk_real, blk_imag)
                else:
                    blk_out = blk(x, s)
