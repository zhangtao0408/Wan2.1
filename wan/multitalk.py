...
try:
    ...
    from inspect import signature as ArgSpec
import logging
import json
import math
import importlib
...
from .utils.utils import profiling_sample
...
if not npu_available:
    ...
    AutoWrappedQLinear = None
    AutoWrappedLinear = None
    AutoWrappedModule = None
    enable_vram_management = None
    logging.warning(f"NPU does not support vram_management, set it to None.")
...
if not npu_available:
    ...
    requantiz = None
    qlinear = None
    logging.warning(f"NPU does not support optimum, set it to None.")
...
class MultiTalkPipeline:
    ...
    def __init__(
        ...
        if npu_available:                      
            self.dit_prof = profiling_sample(
                wait=2,
                warmup=3,
                active=1,
                repeat=1,
    ...
    def enable_vram_management(self, num_persistent_param_in_dit=None):
        if enable_vram_management is None:
            logging.warning(f"NPU does not support enable_vram_management, skip.")
            return
    ...
    def generate(self,
        ...
        while True:
            ...
            with torch.no_grad():
                ...
                video_frames = torch.zeros(1, cond_image.shape[1], frame_num-cond_image.shape[2], target_h, target_w, device=self.device)
            ...
            with torch.no_grad(), no_sync():
                ...
                if self.dit_prof and dist.get_rank() == 0 and sampling_steps >=4:
                    self.dit_prof.start()
                ...
                for i in progress_wrap(range(len(timesteps)-1)):
                    ...
                    if self.dit_prof and dist.get_rank() == 0:
                        self.dit_prof.step()

                dit_time = time.time() - dit_time
                
                if int(os.getenv("RANK", 0)) == 0:
                    print(f"Dit time: {dit_time}")
                if self.dit_prof and dist.get_rank() == 0:
                    self.dit_prof.stop()
